...
class GoRunner:
    ...
    def _clean_code_output(self, output) -> str:
        ...
        # Remove any markdown headers or explanatory text before the package declaration
        if "package" in code:
            lines = code.split('\n')
    ...
    def _extract_code(self, text: str) -> str:
        ...
        # If we find </think>, get everything after the last one
        if "</think>" in text:
            parts = text.split("</think>")
            # Take the last part (after the final </think>)
            text = parts[-1].strip()

        # Now look for code blocks
        if "```" in text:
            # Extract code between the last pair of backticks
            code_blocks = text.split("```")
            for i in range(len(code_blocks)-2, -1, -2):  # Start from last code block
                if i % 2 == 1:  # Odd indices contain code
                    block = code_blocks[i]
                    # Remove language identifier if present
                    if block.startswith("go") or block.startswith("golang"):
                        block = block[2:].strip()
                    return block.strip()
        return text.strip()
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python script.py <config.yaml> [--llm <gemini|ollama|openai|deepseek|llamacpp|groq>] [--ollama-host <host_url>] [--ollama-model <model_name>] [--openai-model <model_name>] [--deepseek-url <url>] [--deepseek-key <key>] [--llama-server-url <url>] [--groq-model <model_name>] [--verbose]           ")
        sys.exit(1)
    ...
    parser.add_argument('--deepseek-url', 
                       default='https://stevef1uk--myid-llama-cpp-server-v1-serve.modal.run', 
                       help='DeepSeek API URL')
    parser.add_argument('--deepseek-key', 
                       default=os.environ.get('DEEPSEEK_KEY'),  # Get from environment variable
                       help='DeepSeek API Key')
    parser.add_argument('--llama-server-url', 
                       default='https://stevef1uk--myid-llama-cpp-server-v1-serve.modal.run', 
                       help='URL for the llama.cpp server')
    ...
    class OllamaLLM(LLM):
        def __init__(self, model, base_url, temperature=0.7, verbose=False, api_key=None):
            ...
            print(f"Initializing OllamaLLM with API key: {self.api_key}")
        ...
        def call(self, prompt: str, **kwargs) -> str:
            ...
            if self.api_key:
                ...
                print(f"Using API key in headers: {self.api_key}")
            ...
            if self.verbose:
                print(f"Converted prompt: {full_prompt}")
